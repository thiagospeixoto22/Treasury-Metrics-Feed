...
def build_email_bodies(metrics: dict, commentary: str, report_tz: str) -> tuple[str, str, str]:
    ...
    <html>
      <body style="font-family: Arial, sans-serif; color: #222;">
        ...
        <p style="margin-top: 0; color: #555;">
          Latest available data date: <strong>{latest_local.strftime('%B %d, %Y')}</strong><br>
          Week-ago comparison date: <strong>{week_local.strftime('%B %d, %Y')}</strong><br>
          Year-ago comparison date: <strong>{year_local.strftime('%B %d, %Y')}</strong>
        </p>
        ...
        <h3 style="margin-top: 24px;">12-Month Yield Chart</h3>
        <img src="cid:treasury_chart" alt="Treasury Yield Chart" style="max-width: 100%; border: 1px solid #ddd;" />
...
Latest available data date: {latest_local.strftime('%B %d, %Y')}
Week-ago comparison date: {week_local.strftime('%B %d, %Y')}
Year-ago comparison date: {year_local.strftime('%B %d, %Y')}
...
Chart attached inline in HTML version.
""".strip()
...
def create_message(sender: str, recipients: list[str], subject: str, html_body: str, text_body: str, chart_bytes: bytes) -> dict:
    ...
    image.add_header("Content-ID", "<treasury_chart>")
    image.add_header("Content-Disposition", "inline", filename="treasury_chart.png")
    root.attach(image)
...
def send_email(service, sender: str, recipients: list[str], subject: str, html_body: str, text_body: str, chart_bytes: bytes) -> None:
    message = create_message(sender, recipients, subject, html_body, text_body, chart_bytes)
...
def main():
    ...
    subject, html_body, text_body = build_email_bodies(metrics, commentary, report_timezone)

    if subject_prefix:
        subject = f"{subject_prefix} {subject}"

    chart_bytes = build_chart(rates_df, metrics["latest_date"], chart_days)
